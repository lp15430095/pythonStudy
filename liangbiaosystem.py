...
label_hospital_name1.place(x=1200/2,y=100,anchor=tk.CENTER)
label_hospital_name2.place(x=1200/2,y=130,anchor=tk.CENTER)
#表名
label_sheet_name=tk.Label(root,text='护士用住院患者观察量表(NOSIE)',font=('宋体',20))
label_sheet_name.place(x=1200/2,y=180,anchor=tk.CENTER)
#双横线
# label_double_line=tk.Label(root,text='-----------------------------------------------------',font=100)
# label_double_line.place(x=1636/2,y=204,anchor=tk.CENTER)

separator=ttk.Separator(root,orient='horizontal')
separator.place(x=1200/2,y=204,width=400,anchor=tk.CENTER)
separator2=ttk.Separator(root,orient='horizontal')
separator2.place(x=1200/2,y=208,width=400,anchor=tk.CENTER)
...
label_sheet_ID.place(x=1200/2,y=220,anchor=tk.CENTER)

#创建frame对象，把下面的表格放在里面
frame=tk.Frame(root,width=1200,height=400)
frame.place(x=50,y=326)

#创建Canvas用于绘制表格1636x2310,并放置在frame中
canvas = tk.Canvas(frame, width=1200, height=400)
...
#填充第一列内容：
#1、创建内容列表：
canvas.create_line(100,10,1000,10,fill='black')
canvas.create_line(100,380,1000,380,fill='black')
canvas.create_line(100,10,100,380,fill='black')
canvas.create_line(1000,10,1000,380,fill='black')
